...
FROM uitvoeringen JOIN inschrijvingen ON uitvoeringen.cursus = inschrijvingen.cursus AND uitvoeringen.begindatum = inschrijvingen.begindatum
WHERE date_part('year', uitvoeringen.begindatum) = 2019
...
SELECT inschrijvingen.cursist, inschrijvingen.cursus FROM inschrijvingen
...
SELECT uitvoeringen.cursus, count(*) AS aantal FROM uitvoeringen
...
SELECT (max(gbdatum) - min(gbdatum)) / 365 AS verschil, round(avg(date_part('year', age(current_date, gbdatum))))  FROM medewerkers;
--
-- S4.7.
...
SELECT count(*) AS aantal_medewerkers, avg(COALESCE(comm, 0)) AS commissie_medewerkers, (SELECT avg(comm) FROM medewerkers WHERE functie = 'VERKOPER') FROM medewerkers;
